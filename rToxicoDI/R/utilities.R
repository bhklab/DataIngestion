...
#'
#' @param path `character` Where the table directories are located. Default
#'   is 'procdata'.
#' @param table `character` The name of the table directory, as generated by
#'   the associated `extract*` method.
#'
#' @return A `list` of `data.table`s, with each item named for the TSet it was
#'   generated from.
#'
#' @import data.table
#' @md
#' @export
loadTSetTables <- function(path='procdata', table) {
    files <- list.files(file.path(path, table), pattern='csv',
        full.names=TRUE)
    if (!length(files) > 1)
        stop(.errorMsg(.context(), 'No files found? Check that your paths are
            correct relative to `getwd()`.'))
